...
try{
    ...
    $source='';$route=@shell_exec('/sbin/ip -4 route get '.escapeshellarg($host).' 2>/dev/null');if(is_string($route)&&preg_match('/\bsrc\s+([0-9.]+)/',$route,$m))$source=$m[1];
    if($source===''){$sock=@stream_socket_client('udp://'.$host.':161',$errno,$errstr,2,STREAM_CLIENT_CONNECT);if(is_resource($sock)){$local=(string)stream_socket_get_name($sock,false);fclose($sock);if(preg_match('/^(.+):\d+$/',$local,$m))$source=trim($m[1],'[]');}}
